...
Result ShaderCursor::getField(const char *name, const char *nameEnd, ShaderCursor &outCursor) const {
    ...
    switch (m_typeLayout->getKind()) {
        ...
        default:
            // If the type is not a structure, constant buffer, or parameter block,
            // then we don't support field access by name.
            //
            return SLANG_E_INVALID_ARG;
    }
...
ShaderCursor ShaderCursor::getElement(GfxIndex index) const {
    ...
    switch (m_typeLayout->getKind()) {
        ...
        default:
            // If the type is not an array, structure, vector, or matrix,
            // then we don't support element access by index.
            //
            return ShaderCursor();
    }
